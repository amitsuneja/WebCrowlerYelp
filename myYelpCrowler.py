    ...
    def read_shop_details(list_of_search_page_urls):
        ...
        for shop_url in list_of_search_page_urls:
            ...
            for shop in shops_list:
                ...
                try:
                    shop_name = shop.find(class_="biz-name js-analytics-click").text.strip().replace(",", "-")
                except (AttributeError, KeyError, TypeError) as ex:
                    #  print(ex)
                    shop_name = "BlankValue"

                try:
                    review_count = shop.find(class_="review-count rating-qualifier").text.strip().replace(",", "-")
                except (AttributeError, KeyError, TypeError) as ex:
                    #  print(ex)
                    review_count = "BlankValue"

                try:
                    phone = shop.find(class_="biz-phone").text.strip().replace(",", "-")
                except (AttributeError, KeyError, TypeError) as ex:
                    #  print(ex)
                    phone = "BlankValue"

                try:
                    address = shop.address.text.strip().replace(",", "-")
                except (AttributeError, KeyError, TypeError) as ex:
                    #  print(ex)
                    address = "BlankValue"

                try:
                    neighbour = shop.find(class_="neighborhood-str-list").text.strip().replace(",", "-")
                except (AttributeError, KeyError, TypeError) as ex:
                    #  print(ex)
        ...
        for key in my_dict:
            ...
            for url in my_dict[key]["comments_url_list"]:
                ...
                try:
                    ...
                    all_inner_div_list = list()

                if not all_inner_div_list:
                    break
                else:
                    for record in all_inner_div_list:
                        ...
                        try:
                            ...
                            this_user_review.append("GHOST_USER")

                        try:
                            this_user_review.append(location.text.strip())
                        except (AttributeError, KeyError, TypeError) as ex:
                            this_user_review.append("GHOST_LOCATION")

                        try:
                            this_user_review.append(date.text.strip()[0:10].rstrip("\n\r"))
                        except (AttributeError, KeyError, TypeError) as ex:
                            this_user_review.append("GHOST_DATE")

                        try:
                            this_user_review.append(review.text.strip())
                        except (AttributeError, KeyError, TypeError) as ex:
                            this_user_review.append("GHOST_COMMENTS")
